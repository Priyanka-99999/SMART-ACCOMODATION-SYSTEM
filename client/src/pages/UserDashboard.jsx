    ...
    <div className="max-w-7xl mx-auto px-4 sm:px-6 lg:px-8 pt-28 pb-10 animate-fade-in">
      <div className="mb-10 pb-6 border-b border-gray-100 flex flex-col sm:flex-row justify-between sm:items-end gap-6">
        <div>
          ...
          <p className="text-gray-500 mt-3 text-lg font-medium">Manage your bookings and preferences</p>
        </div>
        <div className="flex bg-gray-100 p-1.5 rounded-2xl w-full sm:w-auto">
          <button 
            onClick={() => setActiveTab('bookings')} 
            className={`flex-1 sm:flex-none px-6 py-2.5 rounded-xl font-bold text-sm flex items-center justify-center gap-2 transition-all ${activeTab === 'bookings' ? 'bg-white text-primary shadow-sm' : 'text-gray-500 hover:text-gray-900'}`}
          >
            <Calendar className="h-4 w-4" /> Bookings
          </button>
          <button 
            onClick={() => setActiveTab('wishlist')} 
            className={`flex-1 sm:flex-none px-6 py-2.5 rounded-xl font-bold text-sm flex items-center justify-center gap-2 transition-all ${activeTab === 'wishlist' ? 'bg-white text-primary shadow-sm' : 'text-gray-500 hover:text-gray-900'}`}
          >
            ...
            {loading ? (
              ...
              <div className="grid gap-8 md:grid-cols-2 lg:grid-cols-3">
                {bookings.map((booking, index) => (
                  <div key={booking._id} className="bg-white rounded-3xl shadow-sm hover:shadow-xl transition-all duration-300 border border-gray-100/50 overflow-hidden flex flex-col animate-slide-up" style={{ animationDelay: `${index * 100}ms` }}>
                    <div className="p-7 flex-grow">
                      <div className="flex justify-between items-start mb-5">
                        <h3 className="font-extrabold text-xl text-gray-900 line-clamp-1">{booking.propertyId?.title || 'Unknown Property'}</h3>
                        <span className="inline-flex items-center px-3 py-1 rounded-full text-xs font-bold bg-green-50 text-green-700 border border-green-200 uppercase tracking-wider">
                          {booking.status}
                        </span>
                      </div>
                      <div className="text-sm text-gray-600 space-y-3 mb-5 font-medium">
                        ...
                        <p className="flex items-center gap-2 text-gray-500"><Calendar className="h-4 w-4" /> <strong>Check-in:</strong> {new Date(booking.checkInDate).toLocaleDateString()}</p>
                        <p className="flex items-center gap-2 text-gray-500"><Calendar className="h-4 w-4" /> <strong>Check-out:</strong> {new Date(booking.checkOutDate).toLocaleDateString()}</p>
                      </div>
                    </div>
                    <div className="bg-gradient-to-r from-gray-50 to-white px-7 py-5 border-t border-gray-100 flex justify-between items-center mt-auto">
                      <span className="text-gray-500 text-sm font-semibold uppercase tracking-wider">Total Paid</span>
                      <span className="font-extrabold text-xl text-primary drop-shadow-sm">Rs {booking.totalPrice}</span>
                    </div>
                  </div>
                ))}
              </div>
              ...
              <div className="text-center py-20 bg-white rounded-[2rem] border border-gray-100 shadow-sm animate-fade-in">
                ...
                <p className="text-gray-500 mb-8 text-lg">Looks like you haven't booked any accommodations.</p>
